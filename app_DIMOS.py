...
if check_password():
    ...
    with st.sidebar:
        # CORRETTO: cerchiamo .jpg come il file caricato
        p_micro = get_asset_path("logo_microgeo.jpg") 
        if os.path.exists(p_micro): 
            st.image(p_micro, use_container_width=True)
        else:
            st.warning("Logo Microgeo non trovato (verificare estensione .jpg)")
                ...
                if not DOCX_AVAILABLE:
                    ...
                    for l_name in sheets:
                        ...
                        for idx, c_name in enumerate(cols_l):
                            ...
                            # FIX FILLNA
                            serie_final = serie.rolling(5, center=True).mean().bfill().ffill()
                            
                            fig_tmp = go.Figure()
                            fig_tmp.add_trace(go.Scatter(x=t_l, y=serie_final))
