...
def run_plotter():
    ...
    if uploaded_file:
        ...
        # --- VISUALIZZAZIONE (TUA ORIGINALE FUNZIONANTE) ---
                    ...
                    if s in gerarchia[d]:
                        for p in sel_params_v:
                            if p in gerarchia[d][s]:
                                ...
                                fig.add_trace(go.Scatter(x=df_dati[col_t], y=y_v, name=f"{s}-{p}", mode='lines+markers', marker=dict(size=3)))
                                if show_trend:
                                    v_idx = y_v.notna()
                                    if v_idx.sum() > 4:
                                        ...
                                        fig.add_trace(go.Scatter(x=df_dati[col_t], y=poly(df_dati[col_t].apply(lambda x: x.timestamp())),
                                            name=f"Trend {s}-{p}", line=dict(color='red', width=2, dash='dash'), opacity=0.5))
            fig.update_layout(height=600, template="plotly_white", xaxis=dict(rangeslider=dict(visible=True)))
            st.plotly_chart(fig, use_container_width=True)

        # --- STAMPA WORD (SINCRONIZZATA E PRECISA) ---
        st.divider()
        st.subheader("📄 Genera Report Word")
        with st.container(border=True):
            cw1, cw2, cw3 = st.columns(3)
            with cw1: sel_dl_w = st.multiselect("DL (Stampa)", sorted(gerarchia.keys()))
            with cw2:
                s_opts_w = sorted(list(set([s for d in sel_dl_w for s in gerarchia[d].keys()])))
                sel_sens_w = st.multiselect("Sensori (Stampa)", s_opts_w)
            with cw3:
                p_opts_w = sorted(list(set([p for d in sel_dl_w for s in sel_sens_w if s in gerarchia[d] for p in gerarchia[d][s].keys()])))
                sel_params_w = st.multiselect("Grandezze (Stampa)", p_opts_w)

            if st.button("🚀 Genera Report") and WORD_OK:
                doc = Document()
                        ...
                        if s in gerarchia[d]:
                            ...
                            if p_list:
                                ...
                                for p in p_list:
                                    ...
                                    # CREAZIONE GRAFICO MATPLOTLIB SINCRONIZZATO
                                    plt.figure(figsize=(10, 5))
                                    # Usiamo lo stile 'marker' per identificare i punti reali come in Plotly
                                    plt.plot(df_dati[col_t], y_c, label='Dati', color='#1f77b4', marker='o', markersize=2, linestyle='-', linewidth=0.8)
                                    ...
                                    if show_trend:
                                        ...
                                        if v_idx.sum() > 4:
                                            ...
                                            # Calcoliamo il trend esattamente sulle stesse date
                                    ...
                                    plt.legend()
                                    plt.gcf().autofmt_xdate() # Sistema le date in diagonale per non sovrapporle
