...
def run_plotter():
    ...
    if uploaded_file:
        ...
        # --- SEZIONE STAMPA WORD (CON IMMAGINI RISOLTO) ---
        st.divider()
                        ...
                        if s in gerarchia[d]:
                            ...
                            if p_finali:
                                ...
                                for p in p_finali:
                                    ...
                                    doc.add_paragraph(f"Parametro: {p} | Zeri rimosssi: {diag['zeri']} | Outliers Gauss: {diag['gauss']}")
                                    doc.add_picture(img_stream, width=Inches(6))
                
                buf = BytesIO()
                doc.save(buf)
                st.download_button("⬇️ Scarica File Word", buf.getvalue(), "Report_DIMOS_Completo.docx")
