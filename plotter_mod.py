...
# --- MOTORE PULIZIA E PARSING (Originale) ---
...
def run_plotter():
    ...
    if uploaded_file:
        ...
        # --- SEZIONE VISUALIZZAZIONE (Tua originale) ---
        ...
        # --- SEZIONE STAMPA WORD (Con generazione immagini aggiunta) ---
        st.divider()
                        ...
                        if s in gerarchia[d]:
                            ...
                            if p_finali:
                                ...
                                for p in p_finali:
                                    ...
                                    # --- PARTE AGGIUNTA PER IL GRAFICO NEL WORD ---
                                    plt.figure(figsize=(10, 5))
                                    plt.plot(df_dati[col_t], y_c, label=f"{p}", color='#1f77b4', linewidth=1)
                                    if show_trend:
                                        v_idx = y_c.notna()
                                        if v_idx.sum() > 4:
                                            x_ts = df_dati.loc[v_idx, col_t].apply(lambda x: x.timestamp())
                                            ...
                                            plt.plot(df_dati[col_t], poly(df_dati[col_t].apply(lambda x: x.timestamp())), 
                                                     color='red', linestyle='--', label='Trend 3° grado')
                                    plt.title(f"{s} - {p}")
