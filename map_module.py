...
# --- FUNZIONE REPORT WORD (CON FIX LOGO) ---
def genera_report_word(fig, stats, tipo_sel, d_range, sigma, zeri_on):
    doc = Document()
    
    # Gestione Logo con controllo esistenza
    logo_path = "logo_dimos.jpg"
    if os.path.exists(logo_path):
        try:
            doc.add_picture(logo_path, width=Inches(1.5))
        except:
            pass # Salta se l'immagine è corrotta
    ...
    info.add_run(f"Periodo Analizzato: {d_range[0]} - {d_range[1]}\n")
    info.add_run(f"Grandezza Fisica: {tipo_sel}\n")
    info.add_run(f"Impostazioni Filtri: Gauss ({sigma} σ), Rimozione Zeri ({'Attivo' if zeri_on else 'No'})")

    # Inserimento grafico (richiede kaleido nei requirements)
    img_bytes = fig.to_image(format="png", width=1000, height=500, scale=2)
    doc.add_picture(BytesIO(img_bytes), width=Inches(6))

    doc.add_heading('Tabella Diagnostica Valori Rimossi', level=1)
    table = doc.add_table(rows=1, cols=3); table.style = 'Table Grid'
    hdr = table.rows[0].cells
    hdr[0].text, hdr[1].text, hdr[2].text = 'Canale Sensore', 'Zeri Rimossi', 'Outliers Gauss'
...
# --- INTERFACCIA ---
def run_plotter():
    st.header("📉 PLOTTER PRO - Monitoraggio")
    ...
    file_input = st.sidebar.file_uploader("Carica Excel", type=['xlsx', 'xlsm'], key="plt_v_final_fixed")
    ...
    try:
        ...
        # Selezione
        tipo_sel = st.selectbox("Seleziona Grandezza:", sorted(list(set([v['tipo'] for v in mappa.values()]))))
        sensori_scelti = st.multiselect("Seleziona Sensori:", [k for k, v in mappa.items() if v['tipo'] == tipo_sel])
        ...
        if sensori_scelti:
            ...
            for i, s in enumerate(sensori_scelti):
                with cols[i]:
                    canali_disp = mappa[s]["canali"]
                    assi = st.multiselect(f"{s}:", list(canali_disp.keys()), format_func=lambda x: canali_disp[x])
                    for a in assi: final_targets[f"{s} - {canali_disp[a]}"] = a
        ...
        if final_targets:
            ...
            c1, c2 = st.columns(2)
            with c1:
                start_date = st.date_input("Data Inizio:", df_values[col_tempo].min().date())
            with c2:
                end_date = st.date_input("Data Fine:", df_values[col_tempo].max().date())
            ...
            if start_date <= end_date:
                ...
                for label, cid in final_targets.items():
                    ...
                    # Sincronizzazione per evitare ValueError length
                ...
                st.error("Errore: La data di inizio è successiva alla data di fine.")
