...
def get_data_from_excel(file_content):
    ...
    # Ricerca flessibile della colonna temporale
    col_tempo = None
    for c in df_values.columns:
        if 'data' in c.lower() or 'time' in c.lower() or 'ora' in c.lower():
            col_tempo = c
            break
    ...
    if col_tempo:
        ...
        st.error("ERRORE: Non ho trovato la colonna con la Data nel foglio dati!")
...
def run_plotter():
    st.header("📉 PLOTTER PRO - Analisi Dati")
    ...
    file_input = st.sidebar.file_uploader("Carica Excel", type=['xlsx', 'xlsm'], key="plt_v_final_v3")
    ...
    try:
        ...
        if final_targets:
            st.divider()
            c1, c2 = st.columns(2)
            with c1: start_date = st.date_input("Inizio:", df_values[col_tempo].min().date())
            with c2: end_date = st.date_input("Fine:", df_values[col_tempo].max().date())
            
            if start_date <= end_date:
                df_p = df_values[(df_values[col_tempo].dt.date >= start_date) & (df_values[col_tempo].dt.date <= end_date)].copy().reset_index(drop=True)
                fig = go.Figure()
                stats_final = {}
